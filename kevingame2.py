...
def kevingame(hitpoints=100): 
	...
	class Bombe(pygame.sprite.Sprite):
		...
		def update(self,time):
			 ...
			 if self.time > 2.0:
				 print "lexi bum"
				 self.kill()
	class Bombemittel(pygame.sprite.Sprite):
		number = 0
		p2image = pygame.image.load("babytux.png")
		p2image = p2image.convert_alpha()
	...
	class Fragment2(pygame.sprite.Sprite):
		"""a fragment of an exploding Bird"""
		gravity = False # fragments fall down ?
		radius= 10
		def __init__(self, pos):
	...
	bombenmittel = pygame.sprite.Group()
	...
	while mainloop:
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if event.key == pygame.K_DELETE and p1.energie >= 10:
					...
					p1.energie-=10.0 
				if event.key == pygame.K_r and p2.energie >= 20:
					#bomben.add(Bombe(p2.rect.center))
					Docht(Bombemittel(p2.rect.center))
					p2.energie-=20.0
				if event.key == pygame.K_t and p1.energie >= 20:
					#bomben.add(Bombe(p1.rect.center))
					Docht(Bombemittel(p1.rect.center))
					p1.energie-=20.0	 
		...
		for player in playergroup:
			crashgroup = pygame.sprite.spritecollide(player, fragmentgroup2, False, pygame.sprite.collide_circle)
			for fraggi2 in crashgroup:
				player.hitpoints -=10	   
